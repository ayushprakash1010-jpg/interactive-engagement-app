...
export default function EventsPage() {
  ...
  React.useEffect(() => {
    ...
    fetchAdminEvents({ search, status: 'live', page, limit: LIMIT })
      .then((res) => {
        if (cancelled) return;
        setEvents(res.data);
        setMeta(res.meta);
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-ink-secondary">
                Monitor currently live events and force-end sessions if needed.
              </p>
            </div>
            {meta && (
              <span className="text-xs text-ink-muted">
                {meta.total.toLocaleString()} live session{meta.total !== 1 ? 's' : ''}
              </span>
            )}
          </div>
        </div>
              ...
              <tbody className="divide-y divide-border">
                ...
                {!loading && !error && events.map((event) => (
                  <tr
                    ...
                    onClick={() => router.push(`/live-sessions/${event.id}`)}
                  >
