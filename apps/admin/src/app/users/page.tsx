...
import { AdminApiError, fetchAdminUsers } from '@/lib/admin-api';
import type { AdminUserSummary, AdminUserListMeta } from '@/lib/admin-api';
...
function EmptyState({ search }: { search: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-20 text-ink-muted">
      ...
      {search && (
        <p className="text-xs">Try a different name, email, User ID, or Auth0 ID.</p>
      )}
    </div>
  );
}
...
export default function UsersPage() {
  ...
  const [meta, setMeta] = React.useState<AdminUserListMeta | null>(null);
  ...
  React.useEffect(() => {
    ...
    fetchAdminUsers({ search, role: roleFilter || undefined, page, limit: LIMIT })
      .then((res) => {
        if (cancelled) return;
        setUsers(res.data);
        setMeta(res.meta);
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-ink-secondary">
                Search across all platform users. Click a user to view their full profile.
              </p>
            </div>
            {meta && (
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input
              id="user-search"
              type="search"
              placeholder="Name, email, User ID, or Auth0 ID…"
              value={searchInput}
        ...
        <div className="rounded-xl border bg-surface-card shadow-sm overflow-hidden">
          ...
          {meta && meta.totalPages > 1 && (
            <div className="flex items-center justify-between border-t px-4 py-3 text-xs text-ink-muted">
              <span>
                Page {meta.page} of {meta.totalPages} &middot; {meta.total.toLocaleString()} users
              </span>
